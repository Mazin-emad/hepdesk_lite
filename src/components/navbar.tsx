...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b border-border bg-card/95 backdrop-blur-xs">
        ...
        <div className="flex items-center space-x-6">
          ...
          >
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] font-mono tracking-wider uppercase text-muted-foreground -mt-1">
                Dispatch Counter
              </span>
            </div>
          </Link>
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`flex items-center space-x-1.5 rounded-xs px-3 py-1.5 text-xs font-medium transition-colors ${
                    link.active
      ...
      <div className="md:hidden flex items-center justify-around border-t border-border py-1.5 px-4 bg-card">
        {navLinks.map((link) => {
          const Icon = link.icon;
          return (
            <Link
              key={link.href}
              href={link.href}
              className={`flex flex-col items-center space-y-0.5 text-[11px] font-medium py-1 px-2.5 rounded-xs ${
                link.active
        ...
        <Link
          href="/tickets/new"
          className="flex flex-col items-center space-y-0.5 text-[11px] font-medium py-1 px-2.5 text-primary"
        >
          <PlusCircle className="h-3.5 w-3.5" />
          <span>New</span>
        </Link>
